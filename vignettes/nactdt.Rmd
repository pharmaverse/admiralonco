...
To start, all data frames needed for the creation of the new anti-cancer therapy start date (`NACTDT`) should be read into the environment. This will be a company specific process. Some
of the data frames needed may be `CM` and `PR`.
...
To derive the New Anti-Cancer Therapy start date with data from a single source, the function `admiral::derive_vars_merged()` can be used. If numeric date is required, pre-processing using `admiral::derive_vars_dt()` can be used, or if time part is needed, `admiral::derive_vars_dt()` can be used.

```{r message=FALSE}
cm_ext <- derive_vars_dt(
  cm,
  dtc = CMSTDTC,
  new_vars_prefix = "CMST"
)
...
Prior to calling the function, two steps must be performed. To start, an input `date_source` object must be created for each source dataset using `admiral::date_source()`. Within this function, any company-specific subsetting can be passed to the `filter` argument. 

```{r message=FALSE}
cm_date <- date_source(
  dataset_name = "cm",
  filter = CMSCAT == "CHEMOTHERAPY" & CMCAT == "ON TREATMENT" & !is.na(CMSTDT),
  date = CMSTDT
)

pr_date <- date_source(
  dataset_name = "pr",
  filter = PRCAT == "CANCER RELATED" & PRSCAT == "ON TREATMENT" & !is.na(PRSTDT),
  date = PRSTDT
)
```

Lastly, numeric version of the dates will be needed, conversion can be done using `admiral::derive_vars_dt()`, or if time part is needed, `admiral::derive_vars_dtm()`. 
...
pr_ext <- pr %>%
  derive_vars_dt(
    dtc = PRSTDTC,
    new_vars_prefix = "PRST"
  )
```

## Multiple Sources - Derive New-Anti Cancer Start Date and Merge with `ADSL` {#nactdt}

After completion of the prerequisite steps, the new anti-cancer date can be derived while simultaneously adding it to `ADSL`.
...
For deriving parameter for new anti-cancer date from a single source, `admiral::derive_param_extreme_event()` can be used, please consult the `{admiralonco}` [Creating ADRS](adrs.html) for guidance on creating parameters with examples.
