...
Note that only the sections required for this vignette are covered in the following steps. To get a detailed guidance on all the steps, refer the [Creating ADRS (Including Non-standard Endpoints)](adrs.html). 
...
We want to share our experiences and the tools we've developed to help use the GCIG criteria. We hope this will provide a reliable solution for anyone trying to use these guidelines in R programming context.

For more information about GCIG criteria user may visit [Definitions for response and progression in ovarian cancer clinical trials incorporating RECIST 1.1 and CA-125 agreed by the Gynecological Cancer Intergroup (GCIG)](https://pubmed.ncbi.nlm.nih.gov/21270624/)
...
2. **CA-125 Partial Response**: baseline CA-125 CA-125 >= 2 * ULRR, later reduced by 50% but not to normal confirmed at least 4 weeks later.

3. **Stable Disease**: CA-125 level does not meet the criteria for either partial response or progression disease.

4. **Progression**: 

- category A: **CA-125 increased at baseline returning to normal after treatment**: CA-125 >= 2 * ULRR on 2 occasions at least 1 week apart.
- category B: **CA-125 increased at baseline not returning to normal after treatment**: CA-125 >= 2 * nadir value (lowest level reached by CA-125) on 2 occasions at least 1 week apart.
...
- RECIST1.1 tumor size and response as per protocol and mapped in `RS`/`TR`/`TU` SDTM domain.
...
```{r echo=FALSE}
list_resp <- tribble(
  ~"CA-125 response per GCIG", 
  ~"CA-125 response mapped",
...
list_supp <- tibble::tribble(
  ~"QNAM", ~"QLABEL", ~"Purpose", 
  "CA125EFL", "CA-125 response evaluable", "CA-125 > 2x ULRR and no mouse antibodies at baseline",
  "CA2XULRR", "CA-125 pre-treatment 2x ULRR", "CA-125 level at baseline (in range/elevated)",
  "MOUSEANT", "Received mouse antibodies", "Prohibited therapy",
  "CA50RED", ">50% reduction from baseline", "Response indicator",
  "CANORM2X", "CA125 normal, lab increased >=2x ULRR", "PD category A or C",
  "CNOTNORM", "CA125 not norm, lab increased >=2x nadir", "PD category B"
)
...
-   [Confimred RECIST1.1 Overall Response](#covrr11)
...
# GCIG sdtm data
devtools::install_git("https://github.com/pharmaverse/pharmaversesdtm/", ref = "123_rs_ca125")
...
dataset_vignette(
  rs,
  display_vars = exprs(USUBJID, RSTESTCD, RSCAT, RSSTRESC, VISIT, CA125EFL, CA2XULRR, CA50RED, CANORM2X, CNOTNORM, MOUSEANT)
)
```
...
adrs <- derive_vars_merged_lookup(
  ...
  new_vars = exprs(PARAMCD, PARAM, PARAMN, PARCAT1, PARCAT1N, PARCAT2, PARCAT2N),
...
According to GCIG guidelines, assessments after patients have received mouse antibodies should not be considered.
...
dataset_vignette(
  ...
  filter = (PARAMCD == "PDCA125" & !is.na(AVALC))
)
```

## CA-125 Progression Category {#pdcacat}

To obtain additional variables that store the progression category for participants who have progressed, we will create `MCRITy/MCRITyML/MCRITyMN` variables according to the ADAM IG guidelines.
...
adrs <- adrs %>%
  mutate(MCRIT1 = ifelse(PARAMCD == "PDCA125", "PD  Category Group", NA_character_)) %>%
  derive_vars_cat(
...
If criterion to find PD category draws from multiple rows (different parameters or multiple rows for a single parameter) this will require the creation of a new PARAM.
...
dataset_vignette(
  ...
  filter = PARAMCD == "CBORCA" & AVALC != "MISSING"
)
```

## Combined Best Unconfirmed Overall Response {#combubor}

For patients who are measurable by both RECIST1.1 and CA-125 the concept of Combined Best Overall Response is used. In our assumptions, RECIST1.1 response is unconfirmed, and the combined response from `RS` domain is based on that unconfirmed RECIST1.1.
...
dataset_vignette(
  adrs,
  display_vars = exprs(USUBJID, AVISIT, PARAMCD, AVALC, ADT),
  filter = PARAMCD == "BORCA11" & AVALC != "MISSING"
)
```

## Confimred RECIST1.1 Overall Response {#covrr11}

For studies where ORR is one of the primary endpoints there is a need to derive combined response based on confirmed RECIST1.1 and confirmed CA-125 overall responses. To perform this step, there is a need to confirm `CR` and `PR` RECIST1.1 responses by deriving Confimred RECIST1.1 Overall Response parameter.

Following table provides a summary of the `Confimred RECIST1.1 Overall Response` calculation at each time point.

```{r echo=FALSE}
list_resp <- tribble(
  ~"RECIST1.1 response at 1st time point", ~"RECIST1.1 response at 2nd time point",
  ~"Confirmed RECIST1.1 response at 1st time point",
  "CR", "CR", "CR",
  "CR", "PR/SD/PD", "PD",
  "CR", "NE", "NE",
  "PR", "CR/PR", "PR",
...
dataset_vignette(
  adrs,
  display_vars = exprs(USUBJID, AVISIT, PARAMCD, AVALC, ADT),
  filter = PARAMCD == "COVRR11" & AVALC != "MISSING"
)
```
...
Following table provides a summary of the `Combined Overall Response by Investigator (Confirmation of RECIST1.1 Reqired)` calculation at each time point as per GCIG guidelines.

```{r echo=FALSE}
list_resp <- tribble(
  ~"Confirmed RECIST1.1 response", ~"Confirmed CA-125 response", ~"Combined response",
  "CR", "CR", "CR",
  "CR", "PR/SD/NE", "PR",
  "PR/SD/NE", "CR", "PR",
  "PR", "CR/PR/SD/NE", "PR",
...
ovr_cbor <- ovr_cbor %>%
  ...
  mutate(
    PARAMCD = "COVR11CA",
    PARAM = "Combined Overall Response by Investigator (Confirmation of RECIST1.1 Reqired)",
    PARAMN = 8,
    PARCAT1 = "Investigator",
    PARCAT1N = 1,
    PARCAT2 = "Combined",
...
dataset_vignette(
  adrs,
  display_vars = exprs(USUBJID, AVISIT, PARAMCD, AVALC, ADT),
  filter = PARAMCD == "COVR11CA" & AVALC != "MISSING"
)
```
...
dataset_vignette(
  adrs,
  display_vars = exprs(USUBJID, AVISIT, PARAMCD, AVALC, ADT),
  filter = PARAMCD == "CBORCA11" & AVALC != "MISSING"
)
```
