...
Metastatic prostate cancer progression often includes measurements not fully captured by RECIST 1.1 criteria alone. Therefore, the PCWG3 guidelines extend these criteria to include **bone metastases progression**, **PSA progression**, and composite measures combining RECIST 1.1 and PCWG3 rules.
...
The dataset `rs_onco_pcwg3` is expected to already be available, containing soft tissue (`RECIST 1.1`), PSA progression, and bone lesion details. This vignette assumes the full dataset is preprocessed and ready for analysis.
...
dataset_vignette(
  rs,
   display_vars = exprs(USUBJID, RSTESTCD, RSSTRESC, VISIT, RSDTC)
)
```
...
bor_cr <- event(
  ...
  dataset_name = "adrs",  # Reference your dataset explicitly
  condition = AVALC == "CR",  # Complete Response condition
  set_values_to = exprs(AVALC = "CR")
)

bor_pr <- event(
  description = "Partial Response (PR) for Best Overall Response",
  dataset_name = "adrs",  # Reference `adrs` for Partial Response
  condition = AVALC == "PR",
...
adrs <- adrs %>%
  derive_extreme_event(
    ...
    order = exprs(PARAMCD == "OVRLRESPC",ADT,RSSEQ),  # Sort by overall response and chronological date
    mode = "first",              # Select first occurrence for UBOR
    source_datasets = list(
      adrs = adrs  # Reference your dataset directly
    ),
    events = list(
      bor_cr, bor_pr, bor_sd, bor_pd, bor_ne  # Event list defined above
    ),
    set_values_to = exprs(
      PARAMCD = "UBOR",  # Parameter code for UBOR
      PARAM = "Unconfirmed Best Overall Response by Investigator",  # Parameter name for UBOR
      PARAMN = 10        # Parameter hierarchy
    )
  )
```
```{r, echo=FALSE}
dataset_vignette(
  adrs%>%
    filter(PARAMCD== "UBOR"),
  display_vars = exprs(USUBJID, PARAMCD, VISIT, AVISIT, RSDTC, ADT, ADY,AVALC,AVAL)
)
```
...
Both RECIST 1.1 and PCWG3 prescribe the use of a 28-day confirmation period for responses like Complete Response (CR) or Partial Response (PR)
...
```{r, eval=TRUE, include=TRUE, message=FALSE}
adrs <- adrs %>%
  arrange(USUBJID, ADT) %>% # Ensure data is sorted chronologically within subject
  derive_extreme_event(
    by_vars = exprs(USUBJID),             # Calculate CBOR per subject
    order = exprs(ADT,RSSEQ),                   # Chronological ordering using ADT
    mode = "first",                       # Select the first occurrence for CBOR
    source_datasets = list(adrs = adrs),  # Reference the `adrs` dataset containing the AVALC responses
    events = list(cbor_cr, cbor_pr),  # Defined CBOR-related events
    set_values_to = exprs(
      PARAMCD = "CBOR",  # Parameter code for CBOR
      PARAM = "Confirmed Best Overall Response by Investigator",  # Parameter name for CBOR
      PARAMN = 11        # Parameter hierarchy number
    )
  )
```
