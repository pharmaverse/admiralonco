...
- All known bone lesions disappear.  
- No new bone lesions are observed.

Based on **PCWG3 guidelines**, the most appropriate and widely used word for "complete resolution of lesions" would be **"disappearance of lesions."** This aligns with PCWG3’s standardized terminology for Complete Response (CR), which is defined as the **disappearance of all lesions**.


### 2. Partial Response (PR)  
- Improvement in bone scan findings without disappearance of lesions.  
- Note: For subjects classified as PR, progression (PD) cannot be confirmed using the 2+2 rule, as PR excludes the presence of new lesions. Refer to study documentation for further clarification if needed.

### 3. Stable Disease (SD)  
- No progression or insufficient change observed in bone scan findings to meet criteria for PR or PD.  
- Note: PD cannot be confirmed using the 2+2 rule for subjects classified as SD unless new lesions are observed on follow-up imaging.

### 4.Progressive Disease Unconfirmed (PDu)  
-Temporary provisional status indicating suspected progression requiring follow-up confirmation.

### 5.Progressive Disease (PD)  
- Confirmed progression based on two or more new lesions and persistence over subsequent scans .See [**Bone Lesion Confirmation Guidance (The 2+2 Rule)**](#2+2rule) for further details.

### 6.Not Evaluable (NE)  
-Insufficient or missing data for response evaluation.
  
### 7.No Evidence of Disease (NED)
-When no target and non-target lesions are identified at Baseline, and no new lesions are identified on-study.
...
### Bone Lesion Confirmation Guidance (The 2+2 Flare Rule){2+2rule}

**Progression on a Bone Scan:**

Please note: The flare period is generally considered to be the first 8–12 weeks post-baseline, primarily involving the initial post-baseline assessment. Please refer to your study documentation to confirm how the flare period is defined.

1. **Flare Period: Bone Scan Progression Criteria for the First Post-Baseline Scan:**
...
2. **After Flare Period: If No Progression is Detected at the First Assessment:**
...
A patient has a baseline scan showing 8 lesions, and the initial post-baseline scan (outside the flare period) shows no additional lesions. This scan becomes the new reference point for subsequent assessments. At a later scan, conducted 12 weeks post-baseline, 2 new lesions are observed relative to the new reference point, and a follow-up scan performed 6 weeks later confirms their persistence with 2 additional new lesions, totaling 4 new lesions since the new reference point. Progression is confirmed with the progression date recorded as the date of the first scan noting the 2 new lesions.
...
#### Derive Combined Overall Time Point Response by Investigator (`OVRLRESC`) Records referenced from above table.
 ...
 Note:`Non-PD` is changed to `NON_PD` at programming level to match with data
...
adrs <- adrs %>%
  mutate(
    AVAL = case_when(
      ...
      AVALC == "PD" ~ 3,            # Progressive Disease
      AVALC == "PDu" ~ 4,           # Progressive Disease Unconfirmed
      AVALC == "SD" ~ 5,            # Stable Disease
      AVALC == "Non-CR/NON-PD" ~ 6, # Neither Complete Response nor Progressive Disease
      AVALC == "NE" ~ 7,            # Not Evaluable
      AVALC == "NED" ~ 8,           # No Evidence of Disease
      AVALC == "NON-PD" ~ 9,        # Non-Progressive Disease (New Mapping for Consistency)
...
### Create last assesment flag for OVRLRESC for dealing with last recorded assessment `Pdu`
```{r}
adrs <- adrs %>%
  restrict_derivation(
...
pdu_to_sd_event <- event(
  ...
  dataset_name = "adrs",
  condition = PARAMCD == "OVRLRESC" & AVALC == "PDu" & LAST_REC_FL =="Y", # PDu → SD
...
pdu_to_pd_event <- event_joined(
  ...
  set_values_to = exprs(AVALC = "PD")                     # Transition PDu → PD
)
...
adrs <- adrs %>%
  derive_extreme_event(
    ...
    events = list(bor_cr, bor_pr, bor_sd, bor_pd, bor_ne,pdu_to_sd_event,pdu_to_pd_event), # Hierarchical order
...
adrs <- adrs %>%
  derive_extreme_event(
    by_vars = exprs(USUBJID),                 
    events = list(cbor_cr, cbor_pr,bor_sd, bor_pd, bor_ne,pdu_to_pd_event,pdu_to_sd_event),         
