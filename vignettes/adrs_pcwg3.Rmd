...
Please note:
- Some of these events are already defined in `{admiralonco}` (see [Pre-Defined
Response Event Objects](../reference/event_objects.html)). The definitions are
repeated here to show the complete picture.
...
adrs <- adrs %>%
  derive_extreme_event(
    ...
    events = list(bor_cr, bor_pr,bor_sd,pdu_to_sd_event,bor_pd,bor_non_crpd,bor_ne,bor_ned,no_data_missing),
     source_datasets = list(adls = adsl, adrs = adrs %>%filter(PARAMCD == "OVRLRESC")),# Use derived responses (OVRLRESC)
...
adrs <- adrs %>%
  derive_extreme_event(
    by_vars = exprs(USUBJID),                 
    events = list(cbor_cr, cbor_pr,bor_sd,pdu_to_sd_event,bor_pd,bor_non_crpd,bor_ne,bor_ned,no_data_missing),         
    source_datasets = list(adls = adsl, adrs = adrs %>% filter(PARAMCD == "OVRLRESC")),
    tmp_event_nr_var = event_nr,
