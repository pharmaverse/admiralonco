...
**Flare Period:** Bone Scan Progression Criteria for the First Post-Baseline Scan: If two or more new bone lesions are identified at the first post-baseline assessment compared to the baseline scan, and then at the second assessment (conducted at least 6 weeks later), two or more additional new lesions are observed (resulting in a cumulative total of four or more new lesions since the baseline scan), the progression date is set as the date of the initial scan. This process is referred to as the "2+2 rule."

**After Flare Period:** If no progression is detected at the first assessment, the initial post-baseline scan(with in flare period) becomes the new reference point for subsequent scans, If two or more new lesions appear relative to this new baseline, and their persistence or increase in number is confirmed by a follow-up scan at least 6 weeks later, the progression date is recorded as the date of the first scan that noted the appearance of the two new lesions.
...
A patient has a baseline scan showing 8 lesions. The intial post-baseline scan, conducted 12 weeks after baseline (outside the flare period), shows no new lesions, meaning progression is not detected. This scan becomes the new reference point for subsequent assessments.
...
**Reassigning BONERESP to PD/Non-PD**
For the scenario of **Progressive Disease Unconfirmed (PDu)** as mentioned in the footnote: PDu represents a temporary marker of possible Progressive Disease where at least two new bone lesions are present, but an additional scan is required for confirmation. As we do not have new lesions count in the existing data, we confirm PD if the next visit is categorized as PD. It is important to note that modifying source data is **Not advisable**; however, this example is included in this version to illustrate the case. In the upcoming release, this section will be removed, and reassignments should be performed at the **Source level only**.

```{r}
adrs <- derive_var_joined_exist_flag(
  dataset = adrs,                              
  dataset_add = adrs,                          
  by_vars = exprs(USUBJID),                     # Group records by subject
  order = exprs(AVISITN, ADT, RSSEQ),           # Order records chronologically
  new_var = PD_CONFIRM,                         # New flag variable
  join_vars = exprs(AVALC, AVISITN, ADT),       # Variables retained from the join
  join_type = "after",                          # Look for subsequent records
  filter_join = PARAMCD == "BONERESP" & AVALC == "PDu", # Evaluate only PDu records
  first_cond_upper = AVALC.join == "PD",        # Look for PD in subsequent visits
  true_value = "Y"                              # Flag "PDu" as "Y" when confirmed
) %>% 
  mutate(
    AVALC = case_when(
      PD_CONFIRM == "Y" ~ "PD",                 # Update AVALC to "PD" when confirmed
      TRUE ~ AVALC                              # Otherwise, retain the original value
    )
  )
...
adrs <- derive_param_computed(
  dataset = adrs,
  by_vars = exprs(!!!get_admiral_option("subject_keys"), RSEVAL, ADT, ADTF, AVISIT, AVISITN),
...
Use the defined events to derive BOR based on the first occurrence in the adrs dataset, prioritizing responses hierarchically (CR > PR > SD > PD > Non-CR/NON-PD >NON-PD >PDu >NE >NED).
...
adrs <- adrs %>%
  derive_extreme_event(
    ...
    events = list(bor_cr, bor_pr,bor_sd,pdu_to_sd_event,bor_pd,bor_non_cr_non_pd,bor_ne,bor_ned,no_assessment_event)# Hierarchical order
...
adrs <- adrs %>%
  derive_extreme_event(
    by_vars = exprs(USUBJID),                 
    events = list(cbor_cr, cbor_pr,bor_sd,pdu_to_sd_event,bor_pd,bor_non_cr_non_pd,bor_ne,bor_ned,no_assessment_event),         
