...
lb <- pharmaversesdtm::lb_onco_pcwg

#Filter only PSA records
lb <- lb %>% filter(LBTESTCD=="PSA" & !is.na(LBORRES))
...
## Bone Response based on PCWG3 Guidelines
...
#### Define Events for Unconfirmed PSA endpoints

```{r}
#Create dataset with one row per subject for N values 
psa_subj <- adpsa %>%
  group_by(!!!get_admiral_option("subject_keys")) %>%
  arrange(ADT, ADTM) %>%        # first PSA in time
  slice(1) %>%
  ungroup()
...
adrs <- adrs %>%
  derive_extreme_event(
    by_vars = get_admiral_option("subject_keys"),
    events  = list(psa50_y, psa50_n),
    source_datasets = list(
      adpsa    = adpsa,
      psa_subj = psa_subj
    ),
    order = exprs(ADTM),  # ADTM only for ordering
    mode  = "first",
    set_values_to = exprs(
      PARAMCD = "PSA50URS",
...
#### Define Events for Confirmed PSA endpoints
...
psa50_confirmed <- event_joined(
  ...
  join_type    = "after",
  first_cond_upper = !is.na(PCHG.join) &
                     PCHG.join <= -50 &
                     ADT.join >= ADT + 21,
  condition = !is.na(PCHG) & PCHG <= -50,
  set_values_to = exprs(
...
```{r}
## PSA50 and PSA90 Responses (Confirmed)
# 1. PSA50CRS (confirmed >=50% decline)
adrs <- adrs %>%
  derive_extreme_event(
    by_vars = get_admiral_option("subject_keys"),
    events  = list(psa50_confirmed),
    source_datasets = list(adpsa = adpsa),
    order = exprs(ADT),       
    mode  = "first",
    set_values_to = exprs(
      PARAMCD = "PSA50CRS",
...
# 2. PSA90CRS (confirmed >=90% decline)
adrs <- adrs %>%
  derive_extreme_event(
    by_vars = get_admiral_option("subject_keys"),
    events  = list(psa90_confirmed),
    source_datasets = list(adpsa = adpsa),
    order = exprs(ADTM),
    mode  = "first",
    set_values_to = exprs(
