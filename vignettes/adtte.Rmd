...
We advise you consult the `{admiral}` [Creating a BDS Time-to-Event ADaM
...
start date of new anti-cancer therapy or date of surgery. The argument expects a
list of `censor_source()` objects. The earliest date is used as the end of the
observation period and events/censorings occurring after this date are not
considered. If the `end_dates` argument is used, "Overall Response by
...
In the example below the `end_dates` argument is used to restrict the events and
censorings to dates up to the start of a new anti-cancer therapy. `NACTDT` would
be pre-derived as first date of new anti-cancer therapy. See `{admiralonco}`
[Creating and Using New Anti-Cancer Start Date](nactdt.html) for deriving
`NACTDT`. In addition `EOSDT` is used for `end_dates` to populate `EVNTDESC`.
