...
events and censoring times. `{admiral}` supports single events like death (Overall
Survival) or composite events like disease progression or death (Progression Free
Survival). More than one source dataset can be used for the definition of the event
and censoring times.

The majority of the functions used here exist from `{admiral}`, except for the
`onco_tte_sources` helper object, provided as an example from `{admiralonco}`.
...
in `{admiral}` and `{admiralonco}`---are used. An alternative would be to use
`ADEVENT` as input.
...
The table below shows all pre-defined `onco_tte_source` objects which should cover the most common use cases.

```{r echo=FALSE}
knitr::kable(admiral:::list_onco_tte_source_objects())
```

As mentioned in the introduction, each company would create their own version of this with the required filtering
conditions and metadata as per your company approach. An example of a possible deviation could be as follows, where
death is sourced from `ADSL`, instead of `ADRS`, and the `EVNTDESC` is different.
...
An optional step at this stage would be required to enable derivation of duration of response: If using `ADRS` / `ADEVENT`
parameters as input for any response dates (instead of variables in `ADSL`) then you would need to use
`admiral::derive_vars_merged_dt` to add these as temporary variables (e.g. `TEMP_RESPDT`) to be able to feed into
`admiral::derive_param_tte` as the start date. This could also be repeated as needed for IRF and confirmed responses.
...
adrs <- adrs %>%
  derive_vars_merged_dt(
    dataset_add = adrs,
    filter_add = PARAMCD == "RSP" & AVALC == “Y”,
...
The pre-defined objects can be passed directly to `admiral::derive_param_tte()` to create a new time-to-event parameter.
Below shows example calls for Overall Survival (OS), Progression-Free Survival (PFS), and duration of response (note
that this is only derived for responder patients so we have to filter source datasets). Note that the reason for including
a randomization date censor is to catch those patients that never have a tumor assessment.
...
We advise you consult the `{admiral}` ["Creating a BDS Time-to-Event ADaM" vignette]
(https://pharmaverse.github.io/admiral/articles/bds_tte.html) for guidance on the different options available
and more examples.
...
ADaM | Sample Code
---- | --------------
ADTTE | [ad_adtte.R](https://github.com/pharmaverse/admiralonco/blob/main/inst/templates/ad_adtte.R){target="_blank"}
