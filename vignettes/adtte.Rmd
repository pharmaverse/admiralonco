...
The table below shows all pre-defined `onco_tte_source` objects which should cover the most
common use cases.

```{r echo=FALSE}
knitr::kable(admiral:::list_tte_source_objects(package = "admiralonco"))
```
...
(instead of a variable in `ADSL`) then you would need to use `admiral::derive_vars_merged_dt`
to add the response date as a temporary variable (e.g. `TEMP_RESPDT`) to be able to feed
into `admiral::derive_param_tte` as the start date. This could also be repeated as needed
for IRF and confirmed responses.
...
Free Survival (PFS), and duration of response (note that this is only derived for responder
...
(https://pharmaverse.github.io/admiral/articles/bds_tte.html) for further guidance on the
different options available and more examples.
