...
exact specifications such as `PARAM`/`PARAMCD` values will vary depending on
company standards.
...
in `{admiral}` and `{admiralonco}`---are used.
...
These pre-defined objects can be passed directly to `admiral::derive_param_tte()` to create a new time-to-event parameter.
...
The definitions can be passed to `admiral::derive_param_tte()` to create a new time-to-event parameter.
...
subject in the output dataset, another `admiral::censor_source()` object should be
...
for the `set_values_to` parameter in `admiral::event_source()` or `admiral::censor_source()` (see
...
across all `admiral::censor_source()` objects as censoring date.
...
`admiral::call_derivation()` function is useful.
