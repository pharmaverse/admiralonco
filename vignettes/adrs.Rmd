...
dataset_vignette(
  ...
  filter = RSTESTCD == "OVRLRESP" & VISIT == "WEEK 2"
)
```
...
from any input source using `admiral::date_source()`.
...
The function `admiral::convert_date_to_dtm()` can be used to derive `ADTM`. As our test data has
no times collected, here we just set time as `00:00:00`.
...
dataset_vignette(
  adrs,
  display_vars = vars(USUBJID, PARAMCD, ADT, ADTM),
