...
as a patient having had a response or a sustained period of time before first `PD`. In this example the "sustained period"
has been defined as 42 days after randomization date.

```{r}
adrs <- adrs %>%
  derive_param_clinbenefit(
    filter_source = PARAMCD == "OVR" & ANL01FL == "Y",
    source_resp = list(rsp),
    source_pd = list(pd),
    source_datasets = list(adrs = adrs),
...
adrs <- adrs %>%
  derive_param_response(
    filter_source = PARAMCD == "BOR" & ANL01FL == "Y",
    source_pd = list(pd),
    source_datasets = list(adrs = adrs),
    resp_cond = AVALC %in% c(“CR”, “PR”),
    set_values_to = vars(
...
`derive_param_confirmed_response()` and `derive_param_confirmed_bor()` can be used. Here is an example of a BOR parameter
when requiring confirmation.
...
The function `derive_param_merged_exist_flag()` can be used to check whether a patient has measurable disease at baseline,
