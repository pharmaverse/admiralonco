...
### Partial Date Imputation and Deriving `ADT`, `ADTM`, `AVISIT` etc

If your data collection allows for partial dates, you could apply a company-specific imputation rule at this stage
when deriving `ADTM` and `ADT`. For this example, here we impute missing day to last possible date.
...
dataset_vignette(
  adrs,
  display_vars = vars(USUBJID, AVISIT, PARAMCD, PARAM, RSSTRESC, RSDTC, ADT, ADTM)
)
```
...
dataset_vignette(
  adrs,
  display_vars = vars(USUBJID, AVISIT, PARAMCD, PARAM, AVAL, ADT, RANDDT, ANL01FL)
)
```
...
new parameters. The function `admiral::derive_param_first_event()` can be used to find the date of first `PD`.
...
dataset_vignette(
  adrs,
  display_vars = vars(USUBJID, AVISIT, PARAMCD, PARAM, AVAL, ADT, ANL01FL),
  filter = PARAMCD == "PD"
)
```
...
The function `derive_param_response()` can be used to check if a patient had a response for BOR. This parameter in most cases
would be a repeat of the Response parameter, but a more interesting version of this derivation could be in the case where using
"confirmed" BOR (see next step below).
...
dataset_vignette(
  adrs,
  display_vars = vars(USUBJID, PARAMCD, ADT, VISITNUM, AVISIT, ADT, ASEQ),
