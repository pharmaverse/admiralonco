...
The function `admiral::derive_param_first_event()` can be used to find
...
`admiral::derive_param_first_event()` function in that it only looks for
...
The function `derive_param_first_event()` can be used to check if a
...
The function `admiral::derive_param_first_event()` can be used to create
...
The function `derive_param_lasta()` can be used to create a parameter
for last disease assessment. There is flexibility to use `source_pd`
argument to only look up to first `PD` assessment, but in the below
example all assessments are included.
