...
The function `derive_param_clinbenefit()` can then be used to derive the clinical benefit parameter,
which we define as a patient having had a response or a sustained period of time before first `PD`. In
this example the "sustained period" has been defined as 42 days after randomization date.
...
The function `derive_param_bor()` can be used to derive the best overall response parameter.
Similar to the above function you can optionally decide what period would you consider a `SD`
or `NON-CR/NON-PD` as being eligible from. In this example, 42 days after randomization date
has been used again.
...
The function `derive_param_response()` can be used to check if a patient had a response for BOR.
This parameter in most cases would be a repeat of the Response parameter, but more interesting
versions of this derivation could be in the case where using "confirmed" BOR (see later steps).
