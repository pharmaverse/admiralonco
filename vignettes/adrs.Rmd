...
The function `admiral::derive_param_extreme_event()` can be used to find
...
The function `derive_param_response()` can then be used to find the date
of first response. This differs from the
`admiral::derive_param_extreme_event()` function in that it only looks for
events occurring prior to first `PD`. In the below example, the response
condition has been defined as `CR` or `PR`.
...
Note that the above gives pre-defined `AVAL` values of: `"CR" ~ 1`,
`"PR" ~ 2`, `"SD" ~ 3`, `"NON-CR/NON-PD" ~ 4`, `"PD" ~ 5`, `"NE" ~ 6`,
`"MISSING" ~ 7`.
...
Then add the additional argument `aval_fun = aval_resp_new` to the above
`derive_param_bor()` call. Be aware that this will only impact the
`AVAL` mapping, not the derivation of BOR in any way - as the function
derivation relies only on `AVALC` here.

## Derive Best Overall Response of CR/PR Parameter {#bcp}

The function `admiral::derive_param_extreme_event()` can be used to check if a
...
The function `admiral::derive_param_extreme_event()` can be used to create
...
The function `admiral::derive_param_extreme_event()` can be used to create a parameter
for last disease assessment. We need to set `new_var` to a dummy variable here, or
otherwise the original `AVALC` value from the record would be overwritten with `"Y"`.
