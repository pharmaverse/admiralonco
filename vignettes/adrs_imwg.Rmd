...
* IMWG criteria incorporating the FLC test,
* EBMT/Center for International Blood and Marrow Transplant Research criteria.

A specific trial may arbitrarily use either of them or a modified version.

This article describes creating an `ADRS` ADaM in multiple myeloma studies
...
In our example will consider the second scenario.
...
library(remotes)
install_git("https://github.com/pharmaverse/pharmaversesdtm/", ref = "86_rs_imwg")
...
Now we will join `SUPPRS` with `RS` domain.

```{r eval=TRUE}
supprs <- supprs_onco_imwg %>%
  select(USUBJID, IDVARVAL, QNAM, QVAL)

supprs <- transform(supprs, IDVARVAL = as.numeric(IDVARVAL))

rs <- rs %>%
  derive_vars_transposed(
    dataset_merge = supprs,
    by_vars = exprs(USUBJID, RSSEQ = IDVARVAL),
    key_var = QNAM,
    value_var = QVAL
  )
...
dataset_vignette(
  adrs_joined,
  display_vars = exprs(USUBJID, PARAMCD, AVISIT, ADT, AVALC, AVALC.next, 
...
adrs_joined <- adrs_joined %>%
  mutate(
    AVAL.confirmed = aval_resp_new2(AVALC.confirmed)
  )
...
We will then make `AVAL` assignment using the previously defined `aval_resp_new()` function (so it is consistent with `OVR` parameter) remove unnecessary variables and define parameter details.

```{r}
adrs_imwg <- adrs_imwg %>%
  mutate(
    AVAL.best = aval_resp_new(AVALC.confirmed)
  ) %>%
  select (-AVAL, -AVALC, -AVAL.next, -AVALC.next, -AVAL.confirmed,
          -AVALC.confirmed, -FRSTDT) %>%
...
adrs <- adrs %>%
  derive_extreme_event(
    ...
    set_values_to = exprs(
      PARAMCD = "RSP",
      PARAM = "IMWG Response by Investigator (confirmation not required)",
      PARCAT1 = "Investigator",
      PARCAT2 = "IMWG",
      AVAL = yn_to_numeric(AVALC)
    )
...
cb_y_imwg <- event(
  description = paste(
    "Define sCR, CR, VGPR, PR or SD occuring at least 42 days after",
    "randomization as clinical benefit"
  ),
  dataset_name = "ovr",
  condition = AVALC %in% c("sCR", "CR", "VGPR", "PR", "SD") &
    ADT >= RANDDT + 42,
  set_values_to = exprs(AVALC = "Y")
)
```
...
adrs <- adrs %>%
  derive_extreme_event(
    ...
    set_values_to = exprs(
      PARAMCD = "CB",
      PARAM = "IMWG Clinical Benefit by Investigator (confirmation not required)",
      PARCAT1 = "Investigator",
      PARCAT2 = "IMWG",
      AVAL = yn_to_numeric(AVALC)
    ),
...
* CR or better response (`PARAMCD` = `CR`),  
* VGPR or better response (`PARAMCD` =`VGPR`).
...
adrs <- adrs %>%
  derive_extreme_event(
    ...
    set_values_to = exprs(
      PARAMCD = "CRRSP",
      PARAM = "IMWG Complete Response by Investigator (confirmation not required)",
      PARCAT1 = "Investigator",
      PARCAT2 = "IMWG",
      AVAL = yn_to_numeric(AVALC)
    ),
...
adrs <- adrs %>%
  derive_extreme_event(
    ...
    set_values_to = exprs(
      PARAMCD = "VGPRRSP",
      PARAM = "IMWG VGPR Rate by Investigator (confirmation not required)",
      PARCAT1 = "Investigator",
      PARCAT2 = "IMWG",
      AVAL = yn_to_numeric(AVALC)
    ),
...
adrs <- adrs %>%
  derive_extreme_event(
    ...
    events = list(bor_scr, bor_cr, bor_vgpr, bor_pr, bor_mr, bor_sd_imwg, bor_pd,
                  no_data_missing),
    set_values_to = exprs(
      PARAMCD = "BOR",
      PARAM = "IMWG Best Overall Response by Investigator (confirmation not required)",
      PARCAT1 = "Investigator",
      PARCAT2 = "IMWG",
      AVAL = aval_resp_new(AVALC)
    )
