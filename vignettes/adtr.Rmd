...
In the third and fourth step we include the analysis day and date (`ADY`/`ADT`) variables to the sum of diameter parameter dataset.  If there are different analysis day/dates associated with a particular visit, we take the minimum day/date.  In the final step, the analysis flag is set `ANL01FL= Y` where lesions assessed at post-baseline match baseline.  
...
`ADTR`.  We calculate the change from baseline (`CHG`, `CHGNAD`) and percentage change from baseline (`PCHG` , `PCHGNAD`) for the parameter `SDIAM` (using `AVAL` - `BASE`) the variable `NADIR` (using `AVAL` - `NADIR`).
