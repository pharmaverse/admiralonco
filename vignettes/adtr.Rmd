...
Some of the data frames needed may be `ADSL`, `ADRS` ,`RS`, `TU`, `TR`, 'SUPPTU' and 'SUPPTR'.

For example purposes, the SDTM and ADaM datasets (based on CDISC Pilot test
data)---which are included in `{admiral.test}`---are used.  Also, see [Handling of Missing Values] explains why we need to use the [convert_blanks_to_na] function.
...
The below examples show the set up of the parameter details and the `AVAL/ANL01FL` variables.
...
In this vignette, the sum of the diameters is counted across all available lesion measurements, the ANL01FL selects the records that should be used as part of the analysis based on RECIST 1.1.  Subsequently we build on this analysis flag (see ANLxxFL (#analflag)) to demonstrate additional analysis flags as example purposes only. The analysis flags therefore should be setup according to your company specifications.
...
Lesions can split or merge, if this is a consideration then this needs to be taken care of when deciding on the algorithm to use to check that all lesions are measured at post-baseline.
...
multiple observations on the same date and where lesions assessed at
post-baseline match baseline.

NEED TO COME BACK TO THIS BLOCK OF CODE FOR FURTHER COMMENTS - ONCE CLARIFIED WITH STEFAN
...
Once we have Sum of Target Diameters parameter ('SDIAM'), the 'NADIR' is set to the lowest sum of diameters in Analysis Value ('AVAL') before the current observation, where all lesions were assessed (i.e ANL01FL=Y).  The
...
Analysis 02 Flag ('ANL02FL') only includes visits (where analysis date is e.g after randomization date ('RANDDT')) with the lowest NADIR (lowest sum of diameter) so maximum tumor shriekage by using the first non-missing lowest percentage change from baseline ('PCHG').
...
Analysis 03 flag ('ANL03FL') includes all sum of diameters where all lesions were assessed (where 'ANL01FL=Y') until the patient has had PD, indicated by using the 'PDFL' variable.
