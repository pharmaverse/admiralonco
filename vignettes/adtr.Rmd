...
adapted (e.g. update the Evaluator 'TR.TREVAL', 'TU.TUEVAL') to extract Independent 
review facility (IRF)/Blinded Independent Central Review (BICR) data.
...
directly from `TR` or additional parameters could be created in ADTR (or similar) depending on
the additional processing required (e.g. imputation of dates, re-labeling of
visits) and your company specifications.

The majority of the functions used here exist from `{admiral}'. NEED TO UPDATE
...
* [Derive Parameters for Lesion Diameters (LDIAMn, LNDIAMn)](#parameters)
* [Derive Parameter for Sum of Diameters (TSD) and ANL01FL](#tsd)
...
`ADSL` variables (in this example, Randomization date 'RANDDT') used for derivations are selected at this step.  The rest of
...
As an example, here we are keeping the tumor location ('TULOC'), and deriving a new
variable for the tumor group site ('TULOCGR1'). You may want to process other variables like
`ADT`.
...
Set the `AVAL/ANL01FL` variables.
...
## Derive Parameter for Sum of Diameter and Analysis Flag `ANL01FL` {#TSD}

Sum of Target Lesions Diameters is the sum of the diameters for lymph and
...
The analysis flag `ANL01FL` flags where the number of lesions assessed
...
Once we have Sum of Target Diameters parameter ('TSD'), the 'NADIR' is set to the lowest sum of diameters in
...
earlier) is set to baseline value.
...
Analysis 04 flag includes all sum of diameters where all lesions were assessed and additionally those indication PD. 
Use 'PDFL' variable.
