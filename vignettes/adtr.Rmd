...
On the `TR` domain we filter on where tumor asssessment short name `TRTESTCD` is `LDIAM' or 'LPERP`.  Depending on your data collection standards you may need to update the code to filter on ``TRTESTCD=`DIAMETER``. If this is the case then the template code where we derive parameters for lesions diameters should be updated accordingly.
