...
individual lesions sizes could be grouped either by using the analysis date 'ADT' or visit 'AVISIT'.  
...
In the example below, we impute the missing day to the earliest possible date to derive the analysis date 'ADT', and then 
...
flag (ANLxxFL (#analflag) to demonstrate additional analysis flags as example purposes only. The analysis flags therefore 
...
of when deciding on the algorithm to use to check all lesions are measured at post-baseline.
...
Here we create baseline flag 'ABLFL' by selecting all records prior to analysis day 1, and 
then populate baseline value.
