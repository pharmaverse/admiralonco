# filter_pd Test 5: error if invalid source_datasets
    ...
    Code
      filter_pd(dataset = adrs, filter = PARAMCD == "OVR", source_pd = source_pd,
...
# filter_pd Test 6: deprecation message if function is called

    Code
      actual_output <- filter_pd(dataset = adrs, filter = PARAMCD == "OVR",
      source_pd = date_source(dataset_name = "adevent", date = ADT, filter = PARAMCD ==
        "PD", ), source_datasets = list(adevent = adevent))
    Message
      `filter_pd()` was deprecated in admiralonco 1.4.
      i Please use `admiral::filter_relative()` instead.
